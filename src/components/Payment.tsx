...
import { BankIcon, CheckIcon, LockIcon, ShieldIcon, VisaIcon, MastercardIcon, AmexIcon, BoltIcon } from './Icons';
...
type PaymentMethod = 'gpay' | 'koPay' | 'card' | 'eft';
...
const WalletMark: React.FC<{ label: string; className?: string }> = ({ label, className = '' }) => (
    <div className={`w-14 h-14 rounded-2xl bg-white text-black flex items-center justify-center font-black tracking-tight ${className}`}>
        {label}
    </div>
);

const methodCopy: Record<PaymentMethod, { label: string; mark: string; title: string; helper: string; cta: string; secondaryCta: string; powered: string; accent: string }> = {
    gpay: { label: 'GPay', mark: 'GPay', title: 'Pay with GPay', helper: 'Fast, secure and private payments.', cta: 'GPay Subscribe', secondaryCta: 'Use another payment method', powered: 'Powered by GPay', accent: 'from-blue-500 to-cyan-400' },
    koPay: { label: 'KOPay', mark: 'KO', title: 'Pay with KOPay', helper: 'Local secure checkout for African payments.', cta: 'KOPay Subscribe', secondaryCta: 'Use another payment method', powered: 'Powered by KOPay', accent: 'from-emerald-500 to-lime-400' },
    card: { label: 'Card', mark: 'Card', title: 'Debit or Credit Card', helper: 'Pay directly with a saved or new card.', cta: 'Pay by Card', secondaryCta: 'Debit or Credit Card', powered: 'Visa, Mastercard and Amex accepted', accent: 'from-purple-500 to-fuchsia-400' },
    eft: { label: 'EFT / Bank', mark: 'EFT', title: 'Direct Payment EFT', helper: 'Bank transfer with manual activation within 24 hours.', cta: 'Submit EFT Proof', secondaryCta: 'View bank details', powered: 'Direct bank transfer', accent: 'from-orange-500 to-amber-400' },
};
        ...
        <div className="animate-fade-in max-w-7xl mx-auto px-4 sm:px-6 py-8 lg:py-10">
            <div className="grid grid-cols-1 xl:grid-cols-[0.95fr_1.05fr] gap-6 items-start">
                ...
                <section className="rounded-[2rem] border border-blue-400/30 bg-bg-surface/95 p-5 sm:p-7 shadow-2xl shadow-blue-950/20">
                    <div className={`rounded-3xl p-[1px] bg-gradient-to-r ${selectedMethod.accent}`}><div className="rounded-3xl bg-bg-main p-5"><div className="flex items-center gap-4"><WalletMark label={selectedMethod.mark} className="text-sm" /><div><h2 className="text-2xl font-black text-white">{selectedMethod.title}</h2><p className="text-sm text-text-secondary mt-1">{selectedMethod.helper}</p></div></div></div></div>
                    ...
                    <div className="mt-6 rounded-3xl border border-white/10 bg-bg-main/60 p-5 space-y-4">
                        ...
                        <div className="rounded-2xl bg-white/5 p-4">
                            <p className="text-xs text-text-secondary mb-2">Paying with</p>
                            {paymentMethod === 'gpay' && <div className="flex items-center justify-between"><span className="font-bold text-white">GPay wallet selected</span><span className="text-xs font-black text-blue-200 bg-blue-500/10 px-2.5 py-1 rounded-full">GPay</span></div>}
                            {paymentMethod === 'koPay' && <div className="flex items-center justify-between"><span className="font-bold text-white">KOPay wallet selected</span><span className="text-xs font-black text-emerald-200 bg-emerald-500/10 px-2.5 py-1 rounded-full">KOPay</span></div>}
                            {paymentMethod === 'card' && <div className="space-y-3"><div className="flex items-center justify-between"><span className="font-bold text-white">Visa ****4242</span><span className="text-text-secondary">⌄</span></div><div className="flex items-center gap-2"><VisaIcon className="w-9" /><MastercardIcon className="w-9" /><AmexIcon className="w-9" /></div></div>}
                            {paymentMethod === 'eft' && <div className="text-sm text-text-primary space-y-1"><p><strong className="text-white">Bank:</strong> First National Bank of Culture</p><p><strong className="text-white">Account:</strong> AfriTranslate AI Inc.</p><p><strong className="text-white">Reference:</strong> {userEmail}</p></div>}
                        </div>
                    </div>
                    ...
                    <button onClick={completePayment} disabled={isLoading} className={`mt-6 w-full py-4 rounded-full bg-gradient-to-r ${selectedMethod.accent} text-white font-black text-base shadow-lg disabled:opacity-70 flex items-center justify-center gap-2`}>{isLoading ? <Spinner /> : selectedMethod.cta}</button>
                    <button onClick={completePayment} disabled={isLoading} className="mt-3 w-full py-3 rounded-full border border-white/10 text-white font-bold hover:bg-white/10 transition-colors">{selectedMethod.secondaryCta}</button>
