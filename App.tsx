...
const TranslatorApp: React.FC<{ onShowLanding: () => void; initialView?: View; wasSignup?: boolean }> = ({ onShowLanding, initialView = 'chat', wasSignup = false }) => {
    ...
    const renderContent = () => {
        ...
        if (currentView === 'profile' && currentUser) return <div className="h-full overflow-y-auto custom-scrollbar"><ProfileDashboard user={currentUser} onUpdateUser={setCurrentUser} /></div>;
        if (currentView === 'library') return <div className="p-3 max-w-5xl mx-auto h-full overflow-y-auto custom-scrollbar"><Library libraryItems={libraryItems} onSelectExample={() => {}} /></div>;
        if (currentView === 'pricing') return <div className="p-3 max-w-5xl mx-auto h-full overflow-y-auto custom-scrollbar"><Pricing onChoosePlan={(plan) => {setSelectedPlanForPayment(plan); setCurrentView('payment');}} onContactSales={() => setCurrentView('contact')} currentUserPlan={currentUser?.plan || 'Free'} /></div>;
        if (currentView === 'payment') return <div className="p-3 max-w-3xl mx-auto h-full overflow-y-auto custom-scrollbar"><Payment selectedItemName={selectedPlanForPayment} onBack={() => setCurrentView('pricing')} onPaymentSuccess={(plan) => { setSelectedPlanForPayment(plan); setCurrentView('paymentSuccess'); }} /></div>;
        if (currentView === 'paymentSuccess') return <div className="flex items-center justify-center h-full p-4"><PaymentSuccess planName={selectedPlanForPayment} onGoToDashboard={handleNewChat} /></div>;
        if (currentView === 'terms') return <div className="p-3 max-w-3xl mx-auto h-full overflow-y-auto custom-scrollbar"><TermsOfService /></div>;
        if (currentView === 'privacy') return <div className="p-3 max-w-3xl mx-auto h-full overflow-y-auto custom-scrollbar"><PrivacyPolicy /></div>;
        if (currentView === 'contact') return <div className="p-3 max-w-xl mx-auto h-full overflow-y-auto custom-scrollbar"><ContactForm /></div>;
        if (currentView === 'live') return <div className="h-full overflow-hidden"><LiveConversation /></div>;
        if (currentView === 'image') return <ImageGenerator />;
        if (currentView === 'motion') return <VideoGenerator />;
        if (currentView === 'about') return <div className="p-6 h-full overflow-y-auto custom-scrollbar"><AboutPage /></div>;
        if (currentView === 'useCases') return <div className="p-6 h-full overflow-y-auto custom-scrollbar"><UseCasesPage /></div>;
        if (currentView === 'testimonials') return <div className="p-6 h-full overflow-y-auto custom-scrollbar"><TestimonialsPage /></div>;
...
const LandingPage: React.FC<{ initialView?: View; onStart: (view?: View) => void }> = ({ initialView = 'home', onStart }) => {
    const [currentView, setCurrentView] = useState<View>(initialView as any);
...
const App: React.FC = () => {
    ...
    if (!session) {
        ...
        return <LandingPage initialView={appState.initialView as any} onStart={(view) => setAppState({ show: true, initialView: view })} />;
    }
