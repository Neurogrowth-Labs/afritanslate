                        ...
                        {!isVisualMode && (
                            <div className="flex items-center gap-2 px-2 pt-1 pb-2 border-b border-white/5 overflow-x-auto no-scrollbar">
                                ...
                                <div className="ml-auto flex items-center gap-2">
                                    <label className="flex items-center gap-1.5 cursor-pointer hover:bg-white/5 px-2 py-1 rounded transition-colors">
                                        <input type="checkbox" checked={isThinkingMode} onChange={(e) => setIsThinkingMode(e.target.checked)} className="accent-accent w-3 h-3" />
                                        <ThinkingIcon className={`w-3.5 h-3.5 ${isThinkingMode ? 'text-accent' : 'text-text-secondary'}`}/>
                                        <span className="text-[10px] font-bold text-text-secondary uppercase">Nuance+</span>
                                    </label>
                                </div>
                            </div>
                        )}
